...
- **JavaScript**: `pipeline-dashboard.js`
  - Uses `pipeline-local-data.js` → handles sample selection/storage
  - Uses `pipeline-submit-modal.js` → generates and validates commands
  - Uses `pipeline-final-modal.js` → displays final commands and submits jobs

### Job Monitoring Flow
- **HTML**: `job_monitor.html` (job status page)
- **View**: `JobMonitorView` (in `job_monitor.py`)
  - Calls `count_running_jobs()` → gets active counts
  - Calls `update_all_running_jobs()` → refreshes job statuses
- **JavaScript**: `job-monitor.js`
  - Makes API calls to `PipelineApiView.get_job_data()`
  - Makes API calls to `PipelineApiView.check_job_status(demand_id)`
  - Makes API calls to `PipelineApiView.stop_alignment(demand_id)`

### Queue Management Flow
- **HTML**: `queue_management.html` (queue management page)
- **View**: `QueueManagementView` (in `job_monitor.py`)
- **JavaScript**: `queue-management.js`
  - Makes API calls to `/api/queue/process/` in `core/views.py`
  - API uses `submit_sample_from_dashboard()` to process queued jobs

### Failed Jobs Flow
- **HTML**: `failed_jobs.html` (failed jobs page)
- **View**: `FailedJobsView` (in `pipeline.py`)
- **API**: `PipelineApiView.retry_failed_job()` and `PipelineApiView.cancel_failed_job()`
  - Uses `submit_sample_from_dashboard()` to retry failed jobs

### Job Submission Flow
1. User selects samples in `dashboard.html`
2. User clicks "Submit" to open submission modal
3. `pipeline-submit-modal.js` displays selection options
4. User configures submission options
5. `pipeline-final-modal.js` shows final commands
6. API call to `/api/queue/import/` (core/views.py:import_queue)
7. Jobs added to queue_jobs table (with "Ready" or "PENDING" status)
